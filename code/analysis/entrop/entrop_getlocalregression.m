...
function entrop_getlocalregression(ep)
    ...
    for i_participant = 1:nb_participants
        fprintf(['entrop_getlocalregression: participant ',num2str(i_participant),'\n']);
        ...
        for i_map = 2:length(e.smoothedstations{i_participant})
            ...
            for i_trial = 1:length(e.smoothedstations{i_participant}{i_map})
                ...
                % cross values
                cross_path = cross_map(cross_map);
                
                % gaze values
                gaze_path = e.smoothedstations{i_participant}{i_map}{i_trial}{1}(cross_map);
                if any(gaze_path)
                    ok = 1;
                else
                    ok = 0;
                ...
                % entropy values
                entropy_path = entropy_map(cross_map);
                
                % bottleneck values
                bottleneck_path = bottleneck_map(cross_map);
                
                % radius values
                radius_path = radius_map(cross_map);
                
                % localentropy values
                localentropy_path = localentropy_map(i_trial,cross_map);
                
                % localbottleneck values
                localbottleneck_path = localbottleneck_map(i_trial,cross_map);
                
                % localradius values
                localradius_path = localradius_map(i_trial,cross_map);
    ...
    % store regressors
    ep.localregression = struct();
    ep.localregression.beta_c = beta_c;
    ep.localregression.beta_e = beta_e;
    ep.localregression.beta_b = beta_b;
    ep.localregression.beta_r = beta_r;
    ep.localregression.beta_le = beta_le;
    ep.localregression.beta_lb = beta_lb;
    ep.localregression.beta_lr = beta_lr;
    ep.localregression.beta_all = beta_all;
    
    ep.localregression.err_c = err_c;
    ep.localregression.err_e = err_e;
    ep.localregression.err_b = err_b;
    ep.localregression.err_r = err_r;
    ep.localregression.err_le = err_le;
    ep.localregression.err_lb = err_lb;
    ep.localregression.err_lr = err_lr;
    
    ep.localregression.err2_c = err2_c;
    ep.localregression.err2_e = err2_e;
    ep.localregression.err2_b = err2_b;
    ep.localregression.err2_r = err2_r;
    ep.localregression.err2_le = err2_le;
    ep.localregression.err2_lb = err2_lb;
    ep.localregression.err2_lr = err2_lr;
    
    ep.localregression.xx_c = xx_c;
    ep.localregression.xx_e = xx_e;
    ep.localregression.xx_b = xx_b;
    ep.localregression.xx_r = xx_r;
    ep.localregression.xx_le = xx_le;
    ep.localregression.xx_lb = xx_lb;
    ep.localregression.xx_lr = xx_lr;
    ep.localregression.yy_g = yy_g;
end
